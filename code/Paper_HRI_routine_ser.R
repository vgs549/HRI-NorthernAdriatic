...
df <- read.csv(here::here("ser_buildings_stock_dist_encod_elev.csv"))
...
sf_data <- st_as_sf(df_raw, coords = c("longitude_3", "latitude_3"), crs = 4326)
...
scenarios <- list(
  Amenity_Ser = list(
    features = c("encoded_Type_2", "encoded_Green","encoded_Access", "encoded_Pool","length_sea", "msft_heigh", "Price"),
    k = 5,
    m = 1.5
  ),
  Market_Ser = list(
    features = c("encoded_Type_2","stock_cat_1_avg", "encoded_stock_cat_1","Avg_Compr_min_Negozi","area"),
    k = 4,
    m = 1.5
  ),
  Structural_Ser = list(
    features = c("msft_heigh","area","stock_cat_1_avg"),
    k = 4,
    m = 1.5
  )
)
...
# -------------------------------------------------------------------
# Purpose: Test whether service patterns hold under extreme HRI formulations
...
ser_scenarios_map <- c("Amenity_Ser", "Market_Ser", "Structural_Ser")
ser_titles <- c(Amenity_Ser = "Amenity", Market_Ser = "Market", Structural_Ser = "Structural")
ser_panels <- list()

for (sc in ser_scenarios_map) {
...
lisa_block <- (ser_panels$Amenity_Ser_lisa | ser_panels$Market_Ser_lisa | ser_panels$Structural_Ser_lisa) +
  plot_layout(guides = "collect") & theme(legend.position = "bottom")
gi_block <- (ser_panels$Amenity_Ser_gi | ser_panels$Market_Ser_gi | ser_panels$Structural_Ser_gi) +
  plot_layout(guides = "collect") & theme(legend.position = "bottom")
ser_fig <- wrap_elements(lisa_block) | wrap_elements(gi_block)

ggsave("lisa_gi_ser_new.png", plot = ser_fig, width = 20, height = 6, dpi = 300, bg = "white")
cat("\nSaved: lisa_gi_ser_new.png\n")

# ------------------------------
# END OF SCRIPT
# ------------------------------

save.image(here::here("Services_new", "Results_Paper.RData"))